...
def generate_alt_text(image_path):
    """
    Generate alternative text for an image using Azure Vision API.
    :param image_path: Path to the local image file.
    :return: Generated caption text or "No description available" if no captions are found.
    """
    with open(image_path, "rb") as image:

        result = client.describe_image_in_stream(image)
        return result.captions[0].text if result.captions else "No description available"
...
def search():
    ...
    if q == '':
        flash('Enter a keyword about the photo, user, or tag.', 'warning')
    ...
    if category == 'user':
        ...
        pagination = Tag.query.filter(Tag.name.ilike(f"%{q}%")).paginate(page, per_page) 
    else:
        pagination = Photo.query.filter(Photo.description.ilike(f"%{q}%")).paginate(page, per_page)
...
def upload():
    if request.method == 'POST' and 'file' in request.files:
        ...
        # Save the photo and its metadata to the database
        photo = Photo(
            filename=filename,
